...
//  FilmAramaUyg
//
//  Created by Talya Kuvvet on 28.10.2024.
//
...
class DownloaderClient {
    ...
    func filmleriIndir(search: String, completion: @escaping (Result<[Film]?, DownloaderError>) -> Void) {
        ...
        guard let url = URL(string: "https://www.omdbapi.com/?s=\(search))&apikey=3eca5da6") else {
    ...
    func filmDetayiniIndir(imdbID : String, completion : @escaping(Result<FilmDetay,DownloaderError>) -> Void) {
        ...
        guard let url = URL(string: "https://www.omdbapi.com/?i=\(imdbID)&apikey=3eca5da6") else {
            return completion(.failure(.yanlisUrl))
        }
